"""
Aktuality.sk scraper (liberal, Ringier Slovakia) — sitemap-based.

Covers the extended research period 2022-02-01 → 2026-02-23.

Produces a CSV with the same schema as SK_M_aktuality_document_level_with_preds.csv.
portal="Aktuality", illiberal="liberal".

NOTE: Aktuality.sk article URLs follow the pattern:
    https://www.aktuality.sk/clanok/<id>/<slug>/

USAGE
-----
    python scrape_aktuality.py --start 2022-02-01 --end 2026-02-23 \\
        --out ../../data/aktuality_supplement.csv
"""

from __future__ import annotations

import argparse
import re
import sys
from datetime import datetime
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent))

from scraper_utils import PortalConfig, run_portal_scrape

BAD_PARTS = (
    "/tag/", "/tagy/", "/autor/", "/redakcia", "/reklama",
    "/video/", "/galeria/", "/foto/", "/diskusia/",
    "/kontakt", "/podmienky", "/predplatne", "/newsletter", "/rss",
    "/temy/", "/specialne-projekty/",
)

# Aktuality.sk article URL: /clanok/<alphanumeric-id>/<slug>/
# IDs can be numeric (e.g. 711000) or alphanumeric (e.g. 000MmQ8)
_ARTICLE_RE = re.compile(r"/clanok/[a-zA-Z0-9]{4,}/", re.IGNORECASE)


def is_article_url(url: str) -> bool:
    lower = url.lower().split("?", 1)[0].split("#", 1)[0]
    if "aktuality.sk" not in lower:
        return False
    if any(b in lower for b in BAD_PARTS):
        return False
    return bool(_ARTICLE_RE.search(lower))


def main():
    ap = argparse.ArgumentParser(description="Scrape Aktuality.sk via sitemaps.")
    ap.add_argument("--start", required=True, help="start date YYYY-MM-DD (inclusive)")
    ap.add_argument("--end", required=True, help="end date YYYY-MM-DD (inclusive)")
    ap.add_argument("--out", required=True, help="output CSV path")
    ap.add_argument("--sitemap",
                    default="https://www.aktuality.sk/sitemap-articles-time-limited-index.xml",
                    help="root sitemap URL (default: time-limited article index)")
    ap.add_argument("--limit", type=int, default=0,
                    help="max articles to scrape (0 = no limit)")
    ap.add_argument("--no-robots", action="store_true", help="skip robots.txt check")
    args = ap.parse_args()

    config = PortalConfig(
        portal_name="Aktuality",
        portal_code="SK_aktuality",
        illiberal="liberal",
        base_host="www.aktuality.sk",
        root_sitemap=args.sitemap,
        accept_language="sk;q=0.9,cs;q=0.7,en;q=0.5",
        is_article_url=is_article_url,
    )

    run_portal_scrape(
        config,
        date_from=datetime.strptime(args.start, "%Y-%m-%d").date(),
        date_to=datetime.strptime(args.end, "%Y-%m-%d").date(),
        out_path=Path(args.out),
        limit=args.limit,
        check_robots=not args.no_robots,
        sitemap_override=args.sitemap,
    )


if __name__ == "__main__":
    main()
